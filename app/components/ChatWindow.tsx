...
const MessageItem = ({ m }: { m: Message }) => {
  ...
  return (
    <div className={clsx(
      "max-w-[90%] md:max-w-2xl p-4 rounded-2xl text-sm leading-relaxed shadow-sm backdrop-blur-sm border transition-all duration-300 relative", // relative 추가
      m.role === "user" 
        ? "bg-blue-600/90 text-white border-blue-500 rounded-tr-none" 
        : m.role === "system"
          ? "bg-gray-100/50 text-gray-600 text-xs py-1 px-3 border-transparent rounded-full mx-auto"
          : "bg-white/80 text-gray-800 border-white/50 rounded-tl-none shadow-md"
    )}>
      {m.role === "assistant" && (
          <div className="flex items-center gap-2 mb-2 opacity-50 border-b border-gray-200/50 pb-1">
              <Bot size={12} />
      ...
      <div className={clsx(
        ...
        // 접혔을 때 높이 제한
        (!isExpanded && isLongMessage) && "max-h-[140px] overflow-hidden"
      )}>
        <ReactMarkdown 
...
export default function ChatWindow({ messages, isLoading }: ChatWindowProps) {
  ...
  return (
    <main className="flex-1 overflow-y-auto p-4 space-y-4 scroll-smooth scrollbar-hide">
      {messages.map((m, idx) => (
        <motion.div 
          key={idx} 
      ...
      {isLoading && (
        ...
        >
            <div className="bg-white/70 border border-white/50 p-4 rounded-2xl rounded-tl-none shadow-sm flex items-center gap-1">
